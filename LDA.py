...
def getTopics():
    ...
    api_desc_tokens_2Dlist = []

    for index, row in df.iterrows():
        #api_name = row['api_name']
        api_desc_tokens_2Dlist.append(nltk.word_tokenize(row['api_desc']))
        #mydict[api_name] = api_desc_tokens

    #print api_desc_tokens_2Dlist

    dictionary = corpora.Dictionary(api_desc_tokens_2Dlist)
    #print dictionary

    doc_term_matrix = [dictionary.doc2bow(document) for document in api_desc_tokens_2Dlist]
    #print doc_term_matrix

    lda = LdaModel(doc_term_matrix, num_topics=20, id2word=dictionary, passes=50)
